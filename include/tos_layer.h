...
/*
 * Reserved addresses our fake trap handlers live at. These sit right after
...
/* Installs the GEMDOS/BIOS/XBIOS exception vectors and the instruction
 * hook that intercepts them. Must be called after m68k_init(), which
 * resets the instruction hook callback to NULL. */
void tos_layer_init(void);

/* Requests that CPU execution stop as soon as the current trap handler
 * returns. Used by Pterm/Pterm0 to end a "process". */
